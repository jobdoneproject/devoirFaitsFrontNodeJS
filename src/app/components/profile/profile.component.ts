...
export class ProfileComponent implements OnInit {
    ...
    constructor(private userService: UtilisateurApi, public authService: AuthGuard, public router: Router) {
...
// login out from the app
    ...
    removeOfSelection() {
        const index: number = this.weeksToDuplicate.indexOf(moment().year(this.year).week(this.weekNumber).day('monday').startOf('day').unix());
        if (index !== -1) {
    ...
    get isAdministrateur(): boolean {
        return this.userService.getCachedCurrent().privilege === 'Administrateur';
    }
}
